/* =================================================================
 *  ARVORE RUBRO-NEGRA 
 *  Compilar:  gcc -Wall -Wextra -std=c11 -o rubro_negra rubro_negra.c
 *  Executar:  ./rubro_negra
...
/* Dados de um jogador (o "conteudo" guardado em cada no da arvore) */
typedef struct {
    char nome[TAM_NOME];
...
/* ---------------------------------------------------------------
 *  Protótipos das operações da Rubro-Negra 
 * --------------------------------------------------------------- */

NoRB* rb_inserir(NoRB *raiz, Jogador j, int *duplicado) {
    (void) j;
...
NoRB* rb_remover(NoRB *raiz, const char *nome, const char *nacionalidade,
                  const char *data_nascimento, int *removido) {
    (void) nome; (void) nacionalidade; (void) data_nascimento;
    if (removido != NULL) *removido = 0;
    printf("[TODO Entrega 2] rb_remover ainda nao implementado.\n");
    return raiz;
}


NoRB* rb_buscar(NoRB *raiz, const char *nome, const char *nacionalidade,
                 const char *data_nascimento) {
    (void) raiz; (void) nome; (void) nacionalidade; (void) data_nascimento;
    printf("[TODO Entrega 2] rb_buscar ainda nao implementado.\n");
    return NULL;
}
...
/* ---------------------------------------------------------------
 *  main() -- demonstra que a base compila e roda (Entrega 1)
 * --------------------------------------------------------------- */
